    ...
    for name, m in model.model.named_modules():
        if isinstance(m,LlamaMLP):
            ...
            if i == 0:
                ...
                m.matmul1 = NoisyW8A8MatMul(act_quant=act_quant, quantize_output=False)
                
                #m.matmul2 = W8A8MatMul(act_quant=act_quant,quantize_output=True)
                m.matmul2 = NoisyW8A8MatMul(act_quant=act_quant, quantize_output=True)
            else:
                m.q_proj = W8A8Linear.from_float(
                    m.q_proj,
                    weight_quant=weight_quant,
...
for i in range(len(err_prob_list)):
    ...
    print('loading model')
    #model_fp16_normal = AutoModelForCausalLM.from_pretrained('meta-llama/Llama-2-7b-hf', torch_dtype=torch.float16, device_map='auto')
    ...
    #normal_model=quantize_llama_model(model_fp16_normal)
    print('Inject_error...')
